 ...
 * 
 * Creates a live region dynamically, announces after a short delay,
 * then removes it. Pattern from world-map-explorer-v2's accessibility.js.
 * 
 * @param {string} text - The message to announce
 * @param {string} [priority='polite'] - 'polite' or 'assertive'
 * @param {string} [regionIdPrefix='wab-sr'] - Prefix for the generated element ID
 */
function notifySreenReader(text, priority, regionIdPrefix) {
    priority = priority || 'polite';
    regionIdPrefix = regionIdPrefix || 'wab-sr';
    
    var el = document.createElement('div');
    var id = regionIdPrefix + '-' + Date.now();
    ...
    // Use the standard visually-hidden clipping technique
    ...
    // Delay before setting text so AT registers the clear as a distinct event
    window.setTimeout(function () {
        document.getElementById(id).textContent = text;
    }, 100);
    
    // Auto-cleanup after 10 seconds to avoid DOM buildup
    window.setTimeout(function () {
    ...
    console.log(`[SR Notify] (${priority}) ${text}`);
}

// Make notifySreenReader available globally so content scripts and popup can use it
